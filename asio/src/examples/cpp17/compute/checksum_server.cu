...
void checksum(tcp::socket s, yield_context yield)
{
  try
  {
    ...
    for (;;)
    {
      ...
      copy(command_queue, data.begin(), data.end(), device_data.begin(), yield);
      checksum_chunks(command_queue, device_data, device_results, yield);
      reduce_results(command_queue, device_results, yield);
      copy(command_queue, device_results.begin(), device_results.end(), results.begin(), yield);
