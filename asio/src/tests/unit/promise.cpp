...
void promise_all_tester()
{
    ...
    experimental::promise<void(error_code, error_code)> p =
            experimental::promise<>::all(timer1.async_wait(experimental::use_promise),
                                         timer2.async_wait(experimental::use_promise));
...
void promise_race_ranged_tester()
{
    ...
    // promise<std::variant<tuple<error_code, std::size_t>, tuple<error_code, std::size_t>>>
    experimental::promise<void(std::size_t, error_code)> p =
            experimental::promise<>::race(std::array{timer1.async_wait(experimental::use_promise), timer2.async_wait(experimental::use_promise)});
    ...
    experimental::promise<>::race(ctx.get_executor(), std::move(arr)).async_wait([](std::size_t idx) {ASIO_CHECK(idx == -1);});
    ...
    try { experimental::promise<>::race(std::move(arr)); } catch (...) {ex = std::current_exception(); }
    ASIO_CHECK(ex);
}
...
void promise_all_ranged_tester()
{
    ...
    // promise<std::variant<tuple<error_code, std::size_t>, tuple<error_code, std::size_t>>>
    experimental::promise<void(std::vector<error_code>)> p = experimental::promise<>::all(
            std::array{timer1.async_wait(experimental::use_promise), timer2.async_wait(experimental::use_promise)});
    ...
    std::array<experimental::promise<void()>, 0u> arr;
    experimental::promise<>::all(ctx.get_executor(), std::move(arr)).async_wait([](auto v) {ASIO_CHECK(v.size() == 0);});
    ...
    std::exception_ptr ex;
    try { experimental::promise<>::all(std::move(arr)); } catch (...) {ex = std::current_exception(); }
    ASIO_CHECK(ex);
}
...
void promise_cancel_tester()
{
    ...
    // promise<std::variant<tuple<error_code, std::size_t>, tuple<error_code, std::size_t>>>
    experimental::promise<void(error_code, error_code)> p =
            experimental::promise<>::all(timer1.async_wait(experimental::use_promise), timer2.async_wait(experimental::use_promise));
