...
void is_send_buffered_test()
{
  UNIT_TEST_CHECK(!asio::is_send_buffered<asio::stream_socket>::value);

  UNIT_TEST_CHECK(!asio::is_send_buffered<
  ...
  UNIT_TEST_CHECK(!asio::is_send_buffered<test_stream>::value);
...
UNIT_TEST(is_send_buffered_test)
