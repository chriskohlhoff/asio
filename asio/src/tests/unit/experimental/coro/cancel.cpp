...
auto coro_simple_cancel_impl(asio::io_context& ) noexcept -> asio::experimental::coro<void() noexcept, asio::error_code>
{
...
auto coro_throw_cancel_impl(asio::io_context& ) -> asio::experimental::coro<void() , void>
{
    asio::steady_timer timer{
        co_await this_coro::executor,
        std::chrono::seconds(1)};
...
auto coro_simple_cancel_nested_kouter(asio::io_context& ctx, int& cnt) noexcept
    ->  asio::experimental::coro<asio::error_code() noexcept, asio::error_code>
{
    ASIO_CHECK(cnt == 0);
...
void coro_simple_cancel_nested()
{
  ...
  auto k = coro_simple_cancel_nested_k;
