...
public:
  ...
  ASIO_DECL address_v6 netmask() const ASIO_NOEXCEPT;

  /// Obtain an address object that represents the network address.
  address_v6 network() const ASIO_NOEXCEPT
  {
    address_v6::bytes_type addr( address_.to_bytes() );
    address_v6::bytes_type mask( netmask().to_bytes() );
    for ( int i = 0; i < 16; ++i)
        addr[i] &= mask[i];
    return address_v6(addr, address_.scope_id());
  }
  ...
  {
    return network_v6(network(), netmask());
  }
...
/// Create an IPv6 network from an address and netmask.
/**
 * @relates address_v6
 */
inline network_v6 make_network_v6(
    const address_v6& addr, const address_v6& mask)
{
  return network_v6(addr, mask);
}
