...
#include <asio/experimental/coro/allocator.hpp>
#include <asio/experimental/coro/traits.hpp>
#include <asio/experimental/coro/partial.hpp>
#include <asio/experimental/coro/use_coro.hpp>
...
template<>
struct coro_error<error_code>
{
    static error_code invalid()
    { return error::fault; }

    static error_code cancelled()
    { return error::operation_aborted; }

    static error_code interrupted()
    { return error::interrupted; }

    static error_code done()
...
struct coro_error<std::exception_ptr>
{
    ...
    { return std::make_exception_ptr(system_error(coro_error<error_code>::invalid())); }

    static std::exception_ptr cancelled()
    { return std::make_exception_ptr(system_error(coro_error<error_code>::cancelled())); }

    static std::exception_ptr interrupted()
    { return std::make_exception_ptr(system_error(coro_error<error_code>::interrupted())); }

    static std::exception_ptr done()
    { return std::make_exception_ptr(system_error(coro_error<error_code>::done())); }
};
...
struct coro_with_arg
{
    ...
    struct awaitable_t
    {
        ...
        auto await_suspend(coroutine_handle<coro_promise<Y, R, E>> h) -> coroutine_handle<>
        {
            ...
            {
                coro.coro_->awaited_from = dispatch_coroutine(hp.get_executor(), [h]() mutable
                { h.resume(); });
                coro.coro_->reset_error();
                coro.coro_->input_ = std::move(value);
                ...
                return dispatch_coroutine(hp.get_executor(), [h]() mutable
                { h.resume(); });
            }
    ...
    auto async_resume(CompletionHandler &&handler) &&
    {
        return coro.async_resume(std::move(value), std::forward<CompletionHandler>(handler));
    }
...
struct coro
{
    ...
    ~coro()
    {
        if (coro_ != nullptr)
        {
            struct destroyer
            {
                ...
                destroyer(const coroutine_handle<promise_type> &handle) : handle(handle)
                {}
            ...
            auto handle = coroutine_handle<promise_type>::from_promise(*coro_);
            if (handle)
                dispatch(coro_->get_executor(), destroyer{handle});
        }
    }
    ...
    auto async_resume(CompletionHandler &&handler)
    {
        return async_initiate<CompletionHandler, typename traits::completion_handler>(initiate_async_resume(this),
                                                                                      handler);
    }


    template<typename CompletionHandler, detail::convertible_to<input_type> T>
    auto async_resume(T &&ip, CompletionHandler &&handler)
    {
        return async_initiate<CompletionHandler, typename traits::completion_handler>(initiate_async_resume(this),
                                                                                      handler, std::forward<T>(ip));
    }

    auto operator
    co_await() requires (std::is_void_v<input_type>)
    {
  ...
  private:
    ...
    struct awaitable_t
    {
        ...
        template<typename Y, typename R, typename E>
        auto await_suspend(coroutine_handle<detail::coro_promise<Y, R, E>> h) -> coroutine_handle<>
        {
            auto &hp = h.promise();
            ...
            {
                coro_.coro_->awaited_from = detail::dispatch_coroutine(hp.get_executor(), [h]() mutable
                { h.resume(); });
                coro_.coro_->reset_error();
                ...
                return detail::dispatch_coroutine(hp.get_executor(), [h]() mutable
                { h.resume(); });
            }
        }
    ...
    class initiate_async_resume
    {
      public:
        ...
        {
            return [self = self_, h = std::forward<WaitHandler>(handler), exec = std::move(exec)]() mutable
            {
                assert(self);
                auto ch = coroutine_handle<promise_type>::from_promise(*self->coro_);
                assert(ch && !ch.done());
                assert(self->coro_->awaited_from == noop_coroutine());
                    ...
                    std::true_type  /* result is void */) const
        {
            return [self = self_, h = std::forward<WaitHandler>(handler), exec = std::move(exec)]() mutable
            {
                if (!self)
                {
                    post(exec, [h = std::move(h)]() mutable
        ...
        {
            using sig_t = typename traits::completion_handler; //check how to handle the errors
            const auto exec = prefer(get_associated_executor(handler, get_executor()),
...
struct coro_promise final : coro_promise_allocator<coro<Yield, Return, Executor>>,
                                    ...
                                    typename coro_traits<Yield, Return, Executor>::return_type>
{
    ...
    using allocator_type = typename std::allocator_traits<associated_allocator_t < Executor>>::
    ...
    coro_promise(Executor executor, Args &&...) noexcept : executor_(std::move(executor))
    {}

    template<execution_context Context, typename ... Args>
    coro_promise(Context &ctx, Args &&...) noexcept : executor_(ctx.get_executor())
    {}
