#ifndef CORO_USE_CORO_HPP
#define CORO_USE_CORO_HPP
...
struct coro_init_handler
{
    ...
    struct promise_type
    {
        ...
        coro_init_handler get_return_object()
        { return coro_init_handler{this}; }
    ...
    struct noexcept_version
    {
        ...
        template<typename Yield, typename Return, convertible_to <Executor> Executor_>
        auto await_suspend(coroutine_handle<coro_promise<Yield, Return, Executor_> > h) noexcept
        {
    ...
    struct throwing_version
    {
        ...
        template<typename ... Args>
        static auto resume_impl(std::tuple<error_code, Args...> &&tup)
        {
        ...
        template<>
        static auto resume_impl(std::tuple<error_code> &&tup)
        {
            auto ec = get<0>(std::move(tup));
            if (ec)
                asio::detail::throw_exception(system_error(ec, "error_code in use_coro"));
...
{
    using return_type = experimental::detail::coro_init_handler<Executor, typename decay<Args>::type...>;



    template <typename Initiation, typename... InitArgs>
    static return_type initiate(Initiation initiation, experimental::use_coro_t<Executor>, InitArgs... args)
    {
