...
template<typename Coroutine,
         ...
         typename Allocator = typename std::allocator_traits<associated_allocator_t<Executor>>:: template rebind_alloc<std::byte>,
...
struct coro_promise_allocator
{
    ...
    void * operator new(const std::size_t size, Executor executor, Args&& ...)
    {
        ...
        const auto raw = std::allocator_traits<allocator_type>::allocate(alloc, csize);
    ...
    {
        return coro_promise_allocator::operator new(size, ctx.get_executor(), std::forward<Args>(args)...);
    }
    ...
    void operator delete(void* raw, std::size_t size )
    {
        ...
        std::allocator_traits<allocator_type>::deallocate(alloc, static_cast<std::byte*>(raw), size + sizeof(allocator_type));
    }
};

template<typename Coroutine, typename Executor>
struct coro_promise_allocator<Coroutine, Executor, std::allocator<std::byte>, false>
{
...
struct coro_promise_allocator<Coroutine, Executor, Allocator, true>
{
    ...
    template<typename ... Args>
    void * operator new(const std::size_t size, Executor executor, Args&& ...) noexcept
    {
        static_assert(std::is_nothrow_move_constructible_v<allocator_type>);
        allocator_type alloc{get_associated_allocator(executor)};
    ...
    template<execution_context Context, typename ... Args>
    void * operator new(const std::size_t size, Context & ctx, Args&& ... args) noexcept
    {
        return coro_promise_allocator::operator new(size, ctx.get_executor(), std::forward<Args>(args)...);
    }
    ...
    void operator delete(void* raw, std::size_t size ) noexcept
    {
        ...
        std::allocator_traits<allocator_type>::deallocate(alloc, static_cast<std::byte*>(raw) - sizeof(allocator_type), csize);
    }
