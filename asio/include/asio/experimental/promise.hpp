...
struct promise<void(Ts...), Executor>
{
  ...
  /// Check if the promise is completel already
  bool complete() const noexcept
  {
    return impl_ && impl_->done;
  }

  /// Wait for the promise to get ready.
  template <ASIO_COMPLETION_TOKEN_FOR(void(Ts...))
    CompletionToken ASIO_DEFAULT_COMPLETION_TOKEN_TYPE(executor_type)>
  auto async_wait(CompletionToken&& token ASIO_DEFAULT_COMPLETION_TOKEN(executor_type))
  {
    assert(impl_);
...
struct async_result<experimental::use_promise_t<Executor>, R(Args...)>
{
  ...
  using return_type = experimental::promise<void(typename decay<Args>::type...), Executor>;
