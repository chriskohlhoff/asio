...
template<typename T>
concept any_executor = std::is_same_v<std::decay_t<T>, any_io_executor> || execution::is_executor_v<T>;
...
template<typename ... Ts> struct promise_value_type    {using type = std::tuple<Ts...>;};
template<typename T>     struct promise_value_type<T> {using type = T;};
template<>      struct promise_value_type<>  {using type = std::monostate;};
...
struct promise<void(Ts...), Executor>
{
    ...
    bool complete() const { return impl_ && impl_->done; }

    template<typename CompletionHandler>
    auto async_wait(CompletionHandler && handler)
    {
        assert(impl_);

        return async_initiate<CompletionHandler, void(Ts...)>( initiate_async_wait{impl_}, handler);
    }
    promise() = delete;
    ...
    template<any_executor Executor_, is_promise_c ... Ps>
    static auto race(Executor_ exec, Ps ... ps) -> promise<void(std::variant<typename Ps::value_type...>), Executor_>
    {
        using var_t = std::variant<typename Ps::value_type...>;
        using pi = detail::promise_impl<void(var_t), Executor_>;

        struct impl_t : pi
        {
            impl_t(Executor_ exec, Ps && ... ps) : pi(std::move(exec)), tup(std::move(ps)...)
            {
        ...
        auto impl =  std::allocate_shared<impl_t>(get_associated_allocator(exec), exec, std::move(ps)...);
        impl->executor = exec;
        ...
        [impl, exec]<std::size_t... Idx>(std::index_sequence<Idx...>)
        {
            auto step =
                    [&]<std::size_t I>(std::integral_constant<std::size_t, I>)
                    {
                        return [impl]<typename... Args > (Args && ... args)
                        {
                            if (impl->done)
                                return;
                            impl->result = var_t(std::in_place_index<I>, std::forward<Args>(args)...);
                            impl->done = true;
                            if (auto f = std::exchange(impl->completion, nullptr); f != nullptr)
            ...
            (std::get<Idx>(impl->tup).async_wait(bind_executor(exec, step(std::integral_constant<std::size_t, Idx>{}))), ...);
        }(std::make_index_sequence<sizeof...(Ps)>{});
    ...
    template<any_executor Executor_, is_promise_c ... Ps>
    static auto all(Executor_ exec, Ps ... ps) -> promise<void(typename Ps::value_type...), Executor_>
    {
        using pi = detail::promise_impl<void(typename Ps::value_type...), Executor_>;
    ...
    template<any_executor Executor_, typename Range>
    requires requires (Range r) {
        {*std::begin(r)} -> is_promise_c;
        {*std::  end(r)} -> is_promise_c;
    }
    static auto race(Executor_ exec, Range range)
    {

        using var_t = typename std::decay_t<decltype(*std::begin(range))>::value_type;
        using signature_type = std::conditional_t<std::is_same_v<var_t, std::monostate>, void(std::size_t), void(std::size_t, var_t)>;
        using pi = detail::promise_impl<signature_type, Executor_>;
        using promise_t = promise<signature_type, Executor_>;

        struct impl_t : pi
        {
            impl_t(Executor_ exec, Range && range) : pi(std::move(exec)), range(std::move(range))
            {
                this->slot.template emplace<cancel_handler>(this);
            }
        ...
        auto impl =  std::allocate_shared<impl_t>(get_associated_allocator(exec), exec, std::move(range));
        impl->executor = exec;
        ...
        if (size == 0u)
        {
            ...
            impl->done = true;
            if (auto f = std::exchange(impl->completion, nullptr); f != nullptr)
                post(exec, [impl, f = std::move(f)]() mutable {
                    std::apply(std::move(f), std::move(*impl->result));
                });
            ...
            val.async_wait(bind_executor(
                    ...
                    [idx, impl]<typename... Args>(Args &&... args) {
                        ...
                        if constexpr (std::is_same_v<var_t, std::monostate>)
                            ...
                            impl->result = std::make_tuple(idx, var_t(std::forward<Args>(args)...));
                        impl->done = true;
                        if (auto f = std::exchange(impl->completion, nullptr); f != nullptr)
                            std::apply(std::move(f), std::move(*impl->result));
    ...
    static auto all(Executor_ exec, Range  range) -> promise<void(std::vector<typename std::decay_t<decltype(*std::begin(range))>::value_type>), Executor_>
    {
        using var_t = typename std::decay_t<decltype(*std::begin(range))>::value_type;
        using pi = detail::promise_impl<void(std::vector<var_t>), Executor_>;
        ...
        for (auto & val : impl->range) {
            val.async_wait(bind_executor(
                    ...
                    [idx, impl]<typename... Args>(Args &&... args) {
                        ...
                        if (std::all_of(impl->partial_result.begin(), impl->partial_result.end(),
    ...
    static auto race(Range range)
    {
        if (std::begin(range) == std::end(range))
            throw std::logic_error("Can't use race on an empty range with deduced executor");
        else
    ...
    {
        if (std::begin(range) == std::end(range))
            throw std::logic_error("Can't use all on an empty range with deduced executor");
        else
...
private:
    ...
    promise(std::shared_ptr<detail::promise_impl<void(Ts...), Executor>> impl) : impl_(impl) {}
    ...
    struct initiate_async_wait {
        ...
        void operator()(WaitHandler && handler) const
        {
            ...
            if (self_->done)
                post(exec, [self = self_, h = std::forward<WaitHandler>(handler)]() mutable
                {
                    std::apply(std::forward<WaitHandler>(h), std::move(*self->result));
                });
            else
            {
...
{
    using handler_type = experimental::detail::promise_handler<void(typename decay<Args>::type...), Executor>;

    template <typename Initiation, typename... InitArgs>
    static auto initiate(Initiation initiation, experimental::use_promise_t<Executor>, InitArgs... args) -> typename handler_type::promise_type
    {
