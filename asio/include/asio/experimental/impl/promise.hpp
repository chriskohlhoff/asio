...
struct promise_impl<void(Ts...), Executor, Allocator>
{
  ...
  typename aligned_storage<sizeof(result_type), alignof(result_type)>::type result;
  ...
  struct completion_impl final : completion_base
  {
    ...
    void invoke(Ts && ...ts)
    {
      ...
      using alloc_t = typename std::allocator_traits<typename asio::decay<Alloc>::type >
    ...
    completion_impl(Alloc_ && alloc, Handler_ && wh)
        : handler(std::forward<Handler_>(wh)), allocator(std::forward<Alloc_>(allocator))
    {
    }
  };
...
struct promise_handler<void(Ts...), Executor, Allocator>
{
  ...
  promise_handler(Allocator allocator, Executor executor) // get_associated_allocator(exec)
    : impl_{
        std::allocate_shared<promise_impl<void(Ts...), Executor, Allocator>>(allocator, allocator, executor)}
  {
  }
