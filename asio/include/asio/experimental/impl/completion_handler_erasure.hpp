...
//
#ifndef ASIO_EXPERIMENTAL_IMPL_COMPLETION_HANDLER_ERASURE_HPP
#define ASIO_EXPERIMENTAL_IMPL_COMPLETION_HANDLER_ERASURE_HPP
...
namespace asio::experimental::detail
{
...
struct completion_handler_erasure_base<Return(Args...), Executor>
{
    ...
    completion_handler_erasure_base(Executor && executor) : executor(std::move(executor)) {}
...
{
    virtual Return call(Args ...args) override {std::move(func)(std::move(args)...);};
    Func func;
    completion_handler_erasure_impl(Executor && exec, Func && func) :
            completion_handler_erasure_base<Return(Args...), Executor>(std::move(exec)), func(std::move(func)) {}
};
...
struct completion_handler_erasure<Return(Args...), Executor>
{
    ...
    struct deleter_t
    {
        ...
        using allocator_type = typename std::allocator_traits<allocator_base>::template rebind_alloc<completion_handler_erasure_base<Return(Args...), Executor>>;

        allocator_type allocator;
        std::size_t size;

        template<typename Func>
        static std::unique_ptr<completion_handler_erasure_base<Return(Args...), Executor>, deleter_t> make(Executor exec, Func && func)
        {
            using type = completion_handler_erasure_impl<std::remove_reference_t<Func>, Return(Args...), Executor>;
            ...
            auto res =  std::unique_ptr<type, deleter_t>(p, deleter_t{allocator_type(alloc), size});
            std::allocator_traits<alloc_type>::construct(alloc, p, std::move(exec), std::forward<Func>(func));
            return res;
        }

        void operator()(completion_handler_erasure_base<Return(Args...), Executor> * p)
        {
    ...
    completion_handler_erasure& operator=(const completion_handler_erasure &) = delete;
    completion_handler_erasure& operator=(completion_handler_erasure && ) = default;

    constexpr completion_handler_erasure() = default;
    constexpr completion_handler_erasure(nullptr_t) : completion_handler_erasure() {}
    template<typename Func>
    completion_handler_erasure(Executor exec,  Func && func) : impl_(deleter_t::make(std::move(exec), std::forward<Func>(func)))
    {
    }

    constexpr bool operator==(nullptr_t) const {return impl_ == nullptr;}
    constexpr bool operator!=(nullptr_t) const {return impl_ != nullptr;}

private:
    std::unique_ptr<completion_handler_erasure_base<Return(Args...), Executor>, deleter_t> impl_;
};

}

#endif //ASIO_EXPERIMENTAL_IMPL_COMPLETION_HANDLER_ERASURE_HPP
