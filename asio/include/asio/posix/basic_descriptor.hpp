...
public:
  ...
  /// Move-construct a basic_descriptor from a socket of another protocol type.
  /**
   ...
   *
   * @note Following the move, the moved-from object is in the same state as if
   * constructed using the @c basic_descriptor(const executor_type&) constructor.
   */
  template <typename Executor1>
  ...
  /// Move-assign a basic_descriptor from a socket of another protocol type.
  /**
