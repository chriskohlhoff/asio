...
public:
  ...
  ASIO_DECL void start_op(int op_type, socket_type descriptor,
      per_descriptor_data&, reactor_op* op, bool is_continuation,
      ...
      bool is_continuation, bool allow_speculative)
  {
    start_op(op_type, descriptor, descriptor_data, op, is_continuation,
        allow_speculative, &ionotify_reactor::call_post_immediate_completion,
        this);
  }
  ...
  ASIO_DECL void cancel_ops_by_key(socket_type descriptor,
      per_descriptor_data& descriptor_data, int op_type,
      void* cancellation_key);

  // Cancel any operations that are running against the descriptor and remove
  // its registration from the reactor. The reactor resources associated with
  // the descriptor must be released by calling cleanup_descriptor_data.
  ASIO_DECL void deregister_descriptor(
      socket_type descriptor, per_descriptor_data&, bool closing);
...
private:
  ...
  ASIO_DECL int select(asio::detail::mutex::scoped_lock& lock,
  ...
  ASIO_DECL void cancel_ops_unlocked(
      socket_type descriptor, const asio::error_code& ec);
  ...
  struct fdstate
  {
    ...
    fdstate() : ops_(NONE), wanted_(false), armed_(NONE)
    {
      ioev_.sigev_notify = 0;
    }
  };
  using op_bits = fdstate::op_bits;
  static op_bits op_bit(op_types op)
  {
    return static_cast<op_bits>(1 << op);
  }
  ...
  public:
    ...
    ASIO_DECL socket_type set(reactor_op_queue<socket_type>& operations,
        op_queue<operation>& ops, op_types op);
    // Similar to posix_fd_set_adapter::perform but returns the max fd (or invalid_socket)
    ASIO_DECL void perform(reactor_op_queue<socket_type>& operations,
        op_queue<operation>& ops, op_types op);
    bool is_valid(socket_type fd)
    {
      return fd >= 0 && static_cast<size_t>(fd) < size();
    }
  } fdmap_;
