...
public:
  invoker(const implementation_type& impl, Executor& ex)
    : impl_(impl),
      executor_(execution::prefer(ex, execution::outstanding_work.tracked))
  {
  }
  ...
  struct on_invoker_exit
  {
    ...
    ~on_invoker_exit()
    {
      ...
      {
        recycling_allocator<void> allocator;
        execution::require(this_->executor_,
            execution::oneway, execution::single,
...
inline void strand_executor_service::execute(const implementation_type& impl,
    ...
    typename enable_if<
      execution::can_query<Executor, execution::allocator_t<void> >::value
    >::type*)
{
  return strand_executor_service::execute(impl, ex,
      ASIO_MOVE_CAST(Function)(function),
      execution::query(ex, execution::allocator));
}
    ...
    Executor& ex, ASIO_MOVE_ARG(Function) function, const Allocator& a)
{
  ...
  if (execution::query(ex, execution::blocking) != execution::blocking.never
  ...
  if (first)
  {
    execution::require(ex,
        execution::oneway, execution::single
      ).execute(invoker<Executor>(impl, ex));
  }
}
