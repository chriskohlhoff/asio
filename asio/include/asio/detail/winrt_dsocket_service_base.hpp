#ifndef ASIO_DETAIL_WINRT_DSOCKET_SERVICE_BASE_HPP
#define ASIO_DETAIL_WINRT_DSOCKET_SERVICE_BASE_HPP
...
class winrt_dsocket_service_base
{
public:
  ...
  {
    void operator()(Windows::Networking::Sockets::DatagramSocketMessageReceivedEventArgs^ args)
    {
      std::function<void (Windows::Networking::Sockets::DatagramSocketMessageReceivedEventArgs^)> recv_callback;

      {
        asio::detail::mutex::scoped_lock l(recv_mutex_);
        recv_callback_.swap(recv_callback);
        if(!recv_callback)
        {
          recv_queue_.push(args);
          recv_event_.signal(l);
        }
      }
    ...
    std::queue<Windows::Networking::Sockets::DatagramSocketMessageReceivedEventArgs^> recv_queue_;
    asio::detail::event recv_event_;
    asio::detail::mutex recv_mutex_;
    std::function<void (Windows::Networking::Sockets::DatagramSocketMessageReceivedEventArgs^)> recv_callback_;
  };
  ...
  struct base_implementation_type
  {
    ...
    base_implementation_type()
      : socket_(nullptr),
        next_(0),
        prev_(0),
        receive_handler_(new receive_handler())
    {
    }

    // The underlying native socket.
    native_handle_type socket_;
    std::unique_ptr<receive_handler> receive_handler_;
  ...
  // Constructor.
  ASIO_DECL winrt_dsocket_service_base(
      ...
      socket_base::message_flags flags, Handler& handler)
  {
    ...
    start_receive_op(impl,
        buffer_sequence_adapter<asio::mutable_buffer,
          MutableBufferSequence>::first(buffers), nullptr,
        flags, p.p, is_continuation);
    p.v = p.p = 0;
  }
...
protected:
  ...
  // Helper function to perform a bind.
  ...
  // Helper function to perform a synchronous send.
  ASIO_DECL std::size_t do_send_to(
      base_implementation_type& impl, const asio::const_buffer& data,
      const void* addr, socket_base::message_flags flags,
      asio::error_code& ec);

  // Helper function to start an asynchronous send_to operation.
  ASIO_DECL void start_send_to_op(
      base_implementation_type& impl, const asio::const_buffer& data,
      const void* addr, socket_base::message_flags flags,
      winrt_async_op<unsigned int>* op, bool is_continuation);
  ...
  ASIO_DECL void start_receive_op(base_implementation_type& impl,
      const asio::mutable_buffer& data, void* addr, socket_base::message_flags flags,
      winrt_async_op<Windows::Storage::Streams::IBuffer^>* op,
      bool is_continuation);

  ASIO_DECL void finish_receive_op(Windows::Storage::Streams::IBuffer^ data, void* addr,
  ...
  // Helper function to perform a synchronous receive.
  ASIO_DECL std::size_t do_receive_from(base_implementation_type& impl,
...
# include "asio/detail/impl/winrt_dsocket_service_base.ipp"
#endif // defined(ASIO_HEADER_ONLY)

#endif // defined(ASIO_WINDOWS_RUNTIME)

#endif // ASIO_DETAIL_WINRT_DSOCKET_SERVICE_BASE_HPP
